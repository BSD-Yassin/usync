...
- **Performance Optimizations**:
  ...
  - Parallel directory processing (with `parallel` feature) - optimized to avoid directory contention by parallelizing at directory level while serializing file operations within each directory
- **Flexible Options**:
...
- Parallel directory processing optimized using insights from [Fast Unix Commands (FUC)](https://alexsaveau.dev/blog/projects/performance/files/fuc/fast-unix-commands) - directories are processed in parallel while files within each directory are handled sequentially to avoid directory contention
